...
RUN source '/root/.bashrc' \
    ...
    && RUSTFLAGS="-C relocation-model=static -C prefer-dynamic=off -C target-cpu=x86-64-v2 -C target-feature=+crt-static -C link-arg=-fuse-ld=lld" cargo build --bins -j "$(nproc)" --target x86_64-unknown-linux-gnu --no-default-features --features "logging hickory-dns utility server manager multi-threaded aead-cipher-2022 mimalloc" --release --verbose \
...
RUN LDFLAGS="-s" \
    ...
    && export LDFLAGS CXXFLAGS CFLAGS \
    && env \
    && RUSTFLAGS="-C prefer-dynamic=off -C target-cpu=x86-64-v2 -C target-feature=+crt-static" cargo build --bins -j "$(nproc)" --target x86_64-pc-windows-gnu --no-default-features --features "logging hickory-dns dns-over-tls dns-over-https local utility local-dns local-http local-tunnel local-socks4 multi-threaded aead-cipher-2022" --release --verbose \
...
#     && env \
#     && RUSTFLAGS="-C relocation-model=pic -C prefer-dynamic=off -C target-cpu=x86-64-v2 -C target-feature=+crt-static,-vfp2,-vfp3" cargo build --bins -j "$(nproc)" --target armv7-unknown-linux-musleabi --no-default-features --features "logging hickory-dns dns-over-tls dns-over-https local utility local-dns local-http local-tunnel local-socks4 multi-threaded aead-cipher-2022 local-redir local-tun" --release --verbose \
