...
RUN apt-get update && apt-get -y --no-install-recommends install \
    ...
    && ( tmp_dir=$(mktemp -d) && pushd "$tmp_dir" || exit 1 && curl "https://github.com/ninja-build/ninja/releases/download/${ninja_latest_tag_name}/ninja-linux.zip" | bsdtar -xf- && $(type -P install) './ninja' '/usr/bin/' && popd || exit 1 && /bin/rm -rf "$tmp_dir" && dirs -c ) \
