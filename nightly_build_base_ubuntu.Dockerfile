...
RUN apt-get update && apt-get -y --no-install-recommends install \
    ...
    && chmod +x rustup-init \
    && ./rustup-init -y -c rust-src -t x86_64-unknown-linux-gnu x86_64-unknown-linux-musl x86_64-pc-windows-gnu --default-host x86_64-unknown-linux-gnu --profile minimal --default-toolchain nightly --no-modify-path \
    && rm rustup-init \
